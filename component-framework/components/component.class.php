...
Class CT_Component {
		...
		if ( isset($params) && $params ) : 
			...
			foreach ( $params as $param ) : 
										...
										<div class='oxygen-control'>
											...
											<div class="oxygen-select oxygen-select-box-wrapper">
												<div class="oxygen-select-box"
													ng-class="{'oxygen-option-default':iframeScope.isInherited(iframeScope.component.active.id, '<?php echo $param['param_name'] ?>_font-weight')}">
													<div class="oxygen-select-box-current">{{iframeScope.getOption('<?php echo $param['param_name']; ?>_font-weight')}}</div>
													<div class="oxygen-select-box-dropdown"></div>
												</div>
												<div class="oxygen-select-box-options">
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','')">&nbsp;</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','100')">100</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','200')">200</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','300')">300</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','400')">400</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','500')">500</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','600')">600</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','700')">700</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','800')">800</div>
													<div class="oxygen-select-box-option" 
														ng-click="iframeScope.setOptionModel('<?php echo $param['param_name'].'_font-weight';?>','900')">900</div>
												</div>
											</div>
										</div>
									</div>
				...
				elseif ( $param['type'] == 'border' ) : ?>
						...
						<!-- color and size -->
						<div class='oxygen-control-row'>
							<?php $oxygen_toolbar->colorpicker_with_wrapper($param['param_name']."_border-all-color",__("Color","oxygen"),"oxygen-typography-font-color", "ng-show=\"currentBorder=='all'\""); ?>
							<?php $oxygen_toolbar->colorpicker_with_wrapper($param['param_name']."_border-top-color",__("Color","oxygen"),"oxygen-typography-font-color", "ng-show=\"currentBorder=='top'\""); ?>
							<?php $oxygen_toolbar->colorpicker_with_wrapper($param['param_name']."_border-left-color",__("Color","oxygen"),"oxygen-typography-font-color", "ng-show=\"currentBorder=='left'\""); ?>
							<?php $oxygen_toolbar->colorpicker_with_wrapper($param['param_name']."_border-bottom-color",__("Color","oxygen"),"oxygen-typography-font-color", "ng-show=\"currentBorder=='bottom'\""); ?>
							<?php $oxygen_toolbar->colorpicker_with_wrapper($param['param_name']."_border-right-color",__("Color","oxygen"),"oxygen-typography-font-color", "ng-show=\"currentBorder=='right'\""); ?>
					...
					<div class='oxygen-control <?php if ( isset($param['css']) && $param['css'] === false ) echo "oxygen-special-property"; ?><?php if ( isset($param['responsive']) && $param['responsive'] === true ) echo " oxygen-responsive-property"; ?><?php if ( !isset($param['param_name']) || (!in_array($param['param_name'], $options_white_list) && !in_array($param['param_name'], $options_white_list_no_media))) echo " not-available-for-classes"; ?><?php if ( !isset($param['param_name']) || !in_array($param['param_name'], $options_white_list)) echo " not-available-for-media"; ?>'>
						...
						<?php switch ( $param['type'] ) {
							...
							case 'mediaurl' : ?>
								...
								<?php $oxygen_toolbar->mediaurl($param['param_name']); ?>
								<?php break;
	...
	function set_options( $atts ) {
		...
		while(strpos($atts['ct_options'], '[oxygen') !== false && $count < 9) {
			$count++;
			$atts['ct_options'] = preg_replace_callback('/(\")(url|src|map_address|alt|background-image)(\":\"[^\"]*)(\[oxygen[^\]]*\])([^\"\[\s]*)/i', array($this, 'ct_parse_oxy_url'), $atts['ct_options']);
		}
	...
	function build_css($states, $selector, $is_media = false) {
		...
		$media_queries_list_above["page-width"]["minSize"] 	= oxygen_vsb_get_page_width($ct_template_id).'px';
		...
		foreach ( $states as $key => $atts ) {
			...
			if ( $this->options['tag'] == "ct_new_columns" ) {
				...
				if(isset($atts['reverse-column-order']))
					$reverseColumnOrder 			= intval($media_queries_list[$atts['reverse-column-order']]['maxSize']);
			...
			if ( $this->options['tag'] == "oxy_header" ) {
				...
				if ( isset($atts['sticky-header']) && isset($atts['sticky-media']) && 
					...
					$stickyStyles .= "@media (min-width: ".$media_queries_list_above[$atts['sticky-media']]['minSize'].") {";
						...
						$stickyStyles .= '#' . $selector . ".oxy-sticky-header-active .oxygen-hide-in-sticky {";
						$stickyStyles .= "display: none;";
						$stickyStyles .= "}";

						$stickyStyles .= '#' . $selector . " .oxygen-show-in-sticky-only {";
						$stickyStyles .= "display: none;";
						$stickyStyles .= "}";
					if ($atts['sticky-media'] != 'always') {
					$stickyStyles .= "}";
			...
			if ( $this->options['tag'] == "oxy_header_row" ) {
				...
				$showRowStyles .= '.oxy-sticky-header-active > #' . $selector . ".oxygen-show-in-sticky-only {";
			...
			if ($this->options['tag'] == "ct_fancy_icon") {
				...
				if ( $iconStyle == "2" ) {
					if ( isset($iconBackgroundColor) && $iconBackgroundColor ) {
						$styles .= "background-color: " . $iconBackgroundColor . ";\r\n";
						$styles .= "border: 1px solid " . $iconBackgroundColor . ";\r\n";
					}
				}

				if ( $iconStyle == "1" || $iconStyle == "2" ) {
					if(isset($iconPadding)) {
					 	$styles .= "padding: " . $iconPadding . ";";
					}
				}
			...
			if ( isset($atts['text-shadow-color']) ) {
				...
				$hor 	= ($atts['text-shadow-horizontal-offset']) 	? $atts['text-shadow-horizontal-offset']."px " : "";
				$ver 	= ($atts['text-shadow-vertical-offset']) 	? $atts['text-shadow-vertical-offset']."px " : "";
				$blur 	= ($atts['text-shadow-blur']) 				? $atts['text-shadow-blur']."px " : "0px ";
	...
	function validate_shortcode( $atts, $content, $name ) {
	    ...
	    return $oxygen_signature->verify_signature( $name, $atts, $content, true );
    }
