...
Class CT_Nestable_Shortcode extends CT_Component {
	...
	function add_shortcode( $atts, $content, $name ) {
		...
		$wrappingShortcode = base64_decode($options['wrapping_shortcode']);

		$matches = array();

		preg_match('/\[(\w{1,})[^\]]*\]/i', $wrappingShortcode, $matches);
		...
		?><div id="<?php echo esc_attr($options['selector']); ?>" class="<?php if(isset($options['classes'])) echo esc_attr($options['classes']); ?>"><?php echo $wrappedContent;?></div><?php
...
$button = new CT_Nestable_Shortcode ( 
