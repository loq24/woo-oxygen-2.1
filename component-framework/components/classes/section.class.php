...
Class CT_Section extends CT_Component {
	...
	function add_shortcode( $atts, $content, $name ) {
		...
		if (isset($options["video_background"])) {
			...
			$video_html = "<div class='oxy-video-container'><video autoplay loop playsinline muted><source src='{$options['video_background']}'></video><div class='oxy-video-overlay'></div></div>";
		}

		ob_start();
		
		?><<?php echo esc_attr($options['tag'])?> id="<?php echo esc_attr($options['selector']) ?>" class="<?php echo $video_class; ?> <?php if(isset($options['classes'])) echo esc_attr($options['classes']); ?>"><?php echo $video_html; ?><div class="ct-section-inner-wrap"><?php echo do_shortcode( $content ); ?></div></<?php echo esc_attr($options['tag'])?>><?php
