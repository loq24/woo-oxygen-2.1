...
class Oxygen_VSB_Progress_Bar extends CT_Component{
    ...
    function generate_css($params=false, $class=false, $defaults=array()) {
        ...
        <?php echo $params["selector"]; ?> .oxy-progress-bar-background {
            <?php $this->output_single_css_property("background-color", oxygen_vsb_get_global_color_value($params['progress_bar_background_color'])); ?>
        ...
        <?php echo $params["selector"]; ?> .oxy-progress-bar-progress-wrap {
            <?php if(isset($params['progress_bar_progress'])) $this->output_single_css_property("width", $params['progress_bar_progress'], $this->get_css_unit('progress_bar_progress', $params, $defaults)); ?>
        }

        <?php echo $params["selector"]; ?> .oxy-progress-bar-progress {
            <?php $this->output_single_css_property("background-color", oxygen_vsb_get_global_color_value($params['progress_bar_bar_color'])); ?>
            <?php if(isset($params['progress_bar_bar_padding'])) $this->output_single_css_property("padding", $params['progress_bar_bar_padding'], $this->get_css_unit('progress_bar_bar_padding', $params, $defaults)); ?>
            <?php if(isset($stripecss)) echo $stripecss; ?>
            <?php $this->output_single_css_property("animation", implode(", ", $animation_css)); ?>
        }

        <?php echo $params["selector"]; ?> .oxy-progress-bar-overlay-text {
            <?php echo $this->typography_to_css($params, 'progress_bar_left_text_typography', $defaults); ?>
        }

        <?php echo $params["selector"]; ?> .oxy-progress-bar-overlay-percent {
            <?php echo $this->typography_to_css($params, 'progress_bar_right_text_typography', $defaults); ?>
        }
    ...
    function css_output() {
        ...
        $defaultcss = file_get_contents(plugin_dir_path(__FILE__)."progress-bar/progress-bar.css");
                ...
                <div class='oxy-progress-bar-progress-wrap'>
                    <div class='oxy-progress-bar-progress'>
                        <div class='oxy-progress-bar-overlay-text'>
                            <?php echo base64_decode($this->param_array[$options['id']]['progress_bar_left_text']); ?>
                        </div>
                        <div class='oxy-progress-bar-overlay-percent'><?php echo base64_decode($this->param_array[$options['id']]['progress_bar_right_text']); ?></div>
                    </div>
                </div>
            </div>
        </div>
...
$Progress_Bar = new Oxygen_VSB_Progress_Bar( array(
