...
Class Oxy_Google_Maps extends CT_Component {
	...
	function add_shortcode( $atts, $content ) {
		...
		?><div id="<?php echo esc_attr($options['selector']); ?>" class="<?php echo esc_attr($options['classes']); ?>"><iframe src='https://maps.google.com/maps?q=<?php echo urlencode($options['map_address']); ?>&t=m&z=<?php echo $options['map_zoom']; ?>&output=embed&iwloc=near&key=<?php echo get_option("oxygen_vsb_google_maps_api_key", ""); ?>' frameborder=0></iframe></div><?php
...
$oxy_google_maps = new Oxy_Google_Maps( array( 
