...
class oxygen_acf_integration {
	...
	function init() {
		if( !class_exists( 'acf' ) || intval( explode( '.', ACF_VERSION )[0] ) < 5 ) {
			// do nothing 
		} else {
	...
	function download_protected_file( $attachment_id ) {
	    ...
	    $mime_type = $wpdb->get_col( $wpdb->prepare( "SELECT post_mime_type FROM $wpdb->posts WHERE ID=%d;", $attachment_id ) )[0];
	...
	function init_dynamic_data( $dynamic_data ) {
		...
		if( 'ct_template' === $post_type ){
			...
			foreach ( $post_types as $post_type_tmp ) {
		...
		$all_options = array_reduce( $fields, array( $this, "add_button" ) );
		...
		$options_for_url = array_reduce( $fields, array( $this, "add_url_button" ) );
    ...
    function add_url_button( $result, $option ) {
	    $valid_url_field_types = array( 'image', 'text', 'file', 'page-link', 'url', 'link' );
	    $properties = [];
	...
	function acf_content_handler( $atts ){
        global $wpdb;
        $deep_field = false;
        $path = explode( '/', $atts['settings_path'] );
		...
		switch( $field['type'] ) {
            ...
            case 'date_picker':
                ...
                $output = $date->format( empty( $atts[ 'format' ] ) ? $format : $atts[ 'format' ] );
                break;
            case 'select':
            case 'checkbox':
            case 'radio':
            ...
            case 'page_link':
                ...
                foreach ( $field["value"] as $value ) {
                    if( empty( $atts['url_link'] ) ) {
                        ...
                        $output_array[] = '<a href="' . $value . '"' . ( empty( $atts['new_tab'] ) ? '' : ' target="_blank"' ) . '>' . $link_text . '</a>';
                    }
                }
            ...
            case 'link':
                if( empty( $atts['link_link'] ) ) {
                    ...
                    $output = '<a href="' . ( is_array( $field["value"] ) ? $field["value"]["url"] : $field["value"] ) . '"' . ( empty( $atts['new_tab'] ) ? '' : ' target="_blank"' ) . '>' . $link_text . '</a>';
                }
                break;
            case 'true_false':
                if( !empty( $atts['output_custom_onoff'] ) && !empty( $field['ui_on_text'] ) && !empty( $field['ui_off_text'] ) ){
                    $output = $field['value'] ? $field['ui_on_text'] : $field['ui_off_text'];
			...
			case 'google_map':
				if( empty( $this->options['google_maps_key'] ) ) {
					...
					$output = '<div class="acf-map" id="map_' . $field['ID'] . '">';
					$output .= '<div class="marker" data-lat="' . $field['value']['lat'] .'" data-lng="' . $field['value']['lng'] . '"></div>';
					$output .= '</div>';
					$output .= file_get_contents( __DIR__ . '/public/google_map.html' );

					$output = str_replace( 'OXYGEN_GOOGLE_MAPS_KEY', $this->options['google_maps_key'], $output );
					$output = str_replace( 'OXYGEN_MAP_ID', 'map_' . $field['ID'], $output );
				}
				break;
			case 'image':
                $image_id = null;
                ...
                $image_size = explode( 'x', strtolower($atts['size']) );
                ...
                if( is_array( $field['value'] ) ) {
                    ...
                    $image_attachment = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE guid='%s';", $field['value']['url'] ),ARRAY_A );
                    ...
                    $image_attachment = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE guid='%s';", $field['value'] ),ARRAY_A );
                    $image_id = $image_attachment['ID'];
                    $image_url = wp_get_attachment_image_src( $image_id, $image_size )[0];
                }
            ...
            case 'gallery':
                ...
                if( $atts['output_type'] == 'images_id_list' ) {
	...
	function acf_url_handler( $atts ){
	    global $wpdb;
        $field = get_field_object( $atts['settings_path'] );
		$output = '';
		if( empty( $field ) ) return $output;
