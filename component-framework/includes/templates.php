...
function ct_template_builder_settings() { 
			...
			<div class="oxygen-select oxygen-select-box-wrapper">
				...
				<div class="oxygen-select-box-options">
					<div class="oxygen-select-box-option">
						<input ng-model="postsFilter" type="text" value="" placeholder="<?php _e("Search...", "oxygen"); ?>" spellcheck="false"/>
					</div>
	...
	if ( false && get_post_meta( get_the_ID(), 'ct_template_type', true ) != "reusable_part" ) : 
	...
	<div class='oxygen-control-wrapper'>
		<div class='oxygen-control oxygen-template-previewing-control'>
			...
			<div class="oxygen-select oxygen-select-box-wrapper">
				<div class="oxygen-select-box">
					<div class="oxygen-select-box-current" title="{{iframeScope.template.postData.term_name}}">{{iframeScope.template.postData.term_name}}</div>
					<div class="oxygen-select-box-dropdown"></div>
				</div>
				<div class="oxygen-select-box-options">
					<div class="oxygen-select-box-option">
						<input ng-model="termsFilter" type="text" value="" placeholder="<?php _e("Search...", "oxygen"); ?>" spellcheck="false"/>
					</div>
					<div class="oxygen-select-box-option" title="<?php _e("Preview this post", "oxygen"); ?>"
						ng-repeat="term in iframeScope.template.termsList | filter:termsFilter | limitTo:20"
						ng-click="iframeScope.loadTemplatesTerm(term.id, 'archive');">
						{{term.title}}
					</div>
				</div>
			</div>
...
function ct_get_archives_template( $post_id = false) {
	...
	foreach ( $templates->posts as $template ) {
		...
		if ( $applies_to_post_types && is_post_type_archive() ) {
			...
			if ( is_array( $post_types ) && (in_array('all_posttypes', $post_types) || in_array( $post_type, $post_types ) )) {
				return $template;
			}
