...
function parse_shortcodes( $content, $is_first = true, $verify_signature = true ) {
	...
	foreach ( $args as $key => $value ) {
		...
		global $oxygen_signature;
		$verfOptions['ct_options'] 	= json_encode( $options, JSON_FORCE_OBJECT );
		...
		if ( $verify_signature && ! $oxygen_signature->verify_signature( $names[ $key ], $verfOptions, $inner_content[ $key ] ) ) {
