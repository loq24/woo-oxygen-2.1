...
class oxygen_toolset {
	...
	function init_dynamic_data( $dynamic_data ) {
        ...
        $options_for_url_and_images = array_reduce( $fields, array( $this, "add_button_for_url_or_image" ) );
    ...
    private function maybe_override_properties( $properties, $field_type ) {
	    ...
	    switch( $field_type ) {
            ...
            case 'image':
                ...
                $properties[] = array(
                    'name' => __( 'Resize', 'oxygen-toolset' ),
                    'data' => 'resize',
                    'type' => 'select',
                    'options'   => array( __( 'Crop', 'oxygen-toolset' ) => 'crop', __( 'Proportional', 'oxygen-toolset' ) => 'proportional', __( 'Stretch', 'oxygen-toolset' ) => 'stretch' )
                );
                $properties[] = array(
                    'name' => __( 'Proportional. False=image will be cropped to specified height and width. Overridden if size is set', 'oxygen-toolset' ),
                    'data' => 'proportional',
                    'type' => 'select',
                    'options'   => array( __( 'True', 'oxygen-toolset' ) => 'true', __( 'False', 'oxygen-toolset' ) => 'false' )
                );
                $properties[] = array(
                    'name' => __( 'URL. True=output the url of the image instead of the html img tag. Works with the size attribute to output the url of the re-sized image', 'oxygen-toolset' ),
                    'data' => 'url',
                    'type' => 'select',
                    'options'   => array( __( 'True', 'oxygen-toolset' ) => 'true', __( 'False', 'oxygen-toolset' ) => 'false' )
                );
