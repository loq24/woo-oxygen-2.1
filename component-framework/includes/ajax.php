...
function oxygen_vsb_cache_unvirsal_css() {
	...
	if (get_option("oxygen_vsb_universal_css_cache")=='true') {
		...
		$universal_css .= oxygen_vsb_get_classes_styles($fake_properties);
		$universal_css .= oxygen_vsb_get_stylesheet_styles();
		$universal_css .= oxygen_vsb_get_custom_selectors_styles($fake_properties);
...
function ct_setup_default_data() {
	...
	switch($type) {
		case 'Colors':
			ct_setup_default_colors($site, $delete);
...
function ct_setup_default_colors($site, $delete = false) {

	$colors_url = $site.'/wp-json/oxygen-vsb-connection/v1/colors/';



	$result = ct_new_api_remote_get($colors_url);
	$colors = false;
	$lookupTable = false;
	...
	public function callback($matches) {
		...
		if($newColorId === false) {
			foreach($oxygen_vsb_global_colors['colors'] as $eKey => $eColor) {
				...
				if(strtolower($eColor['sourceVal']) == $value && $eColor['set'] === $existingSetId) {
