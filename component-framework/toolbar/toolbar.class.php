...
Class CT_Toolbar {
	...
	function settings_colors() { ?>
		...
		<div ng-repeat="(setID,set) in iframeScope.globalColorSets.sets">
			<div class="oxygen-sidebar-advanced-subtab" 
				ng-click="$parent.switchChildTab('settings', 'colors', set.id);"
...
//		} 
	...
	function ct_show_page_settings() { 
		...
		<div class="oxygen-control-row">
			<div class='oxygen-control-wrapper'>
				...
				<div class='oxygen-measure-box'>
					<input type="text" spellcheck="false"
						placeholder="{{iframeScope.pageSettings['max-width']}}" 
						ng-model="iframeScope.pageSettingsMeta['max-width']"
				...
				<div class="oxygen-select oxygen-select-box-wrapper">
					<div class="oxygen-select-box">
						<div class="oxygen-select-box-current"
							ng-class="{'oxygen-option-default':!iframeScope.pageSettingsMeta['overlay-header-above']}">{{iframeScope.getMediaTitle(iframeScope.pageSettingsMeta['overlay-header-above'],true) || iframeScope.getMediaTitle(iframeScope.pageSettings['overlay-header-above'],true)}}</div>
						<div class="oxygen-select-box-dropdown"></div>
					</div>
					<div class="oxygen-select-box-options">
						<div class="oxygen-select-box-option" 
							ng-click="iframeScope.pageSettingsMeta['overlay-header-above']='';iframeScope.pageSettingsUpdate()"
							ng-class="{'oxygen-select-box-option-active':iframeScope.pageSettingsMeta['overlay-header-above']==''}">
							&nbsp;
						</div>
						<div class="oxygen-select-box-option" 
							ng-click="iframeScope.pageSettingsMeta['overlay-header-above']='never';iframeScope.pageSettingsUpdate()"
							ng-class="{'oxygen-select-box-option-active':iframeScope.pageSettingsMeta['overlay-header-above']=='never'}">
							<?php _e("Never","oxygen"); ?>
						</div>
						<div class="oxygen-select-box-option" 
							ng-repeat="name in iframeScope.sortedMediaList(true)"
							ng-if="name!='default'"
							ng-click="iframeScope.pageSettingsMeta['overlay-header-above']=name;iframeScope.pageSettingsUpdate()"
							ng-class="{'oxygen-select-box-option-active':iframeScope.pageSettingsMeta['overlay-header-above']==name}">
							{{iframeScope.getMediaTitle(name, true)}}
						</div>
						<div class="oxygen-select-box-option" 
							ng-click="iframeScope.pageSettingsMeta['overlay-header-above']='always';iframeScope.pageSettingsUpdate()"
							ng-class="{'oxygen-select-box-option-active':iframeScope.pageSettingsMeta['overlay-header-above']=='always'}">
	...
	function colorpicker_with_wrapper($option,$label = false,$id="",$html_attr="", $deepProperty = false, $wrapperClass=false) { ?>

		<div class='oxygen-control-wrapper<?php echo $wrapperClass?' '.$wrapperClass:'';?>'<?php echo !empty($id)?"id='".$id."' ":''; ?><?php echo $html_attr; ?>>
		<?php if($label) { ?>
			<label class='oxygen-control-label'><?php echo $label; ?></label>
		<?php } ?>
			<div class='oxygen-control'>
		...
		<div class='oxygen-color-picker'
			...
			<div class="oxygen-color-picker-color">
				<input ctiriscolorpicker=""
					...
					ng-model="$parent.iframeScope.component.options[$parent.iframeScope.component.active.id]['model']['<?php echo esc_attr( $option ); ?>']<?php echo $deepProperty?$deepProperty:'';?>"
					ng-style="{'background-color':$parent.iframeScope.component.options[$parent.iframeScope.component.active.id]['model']['<?php echo esc_attr( $option ); ?>']<?php echo $deepProperty?$deepProperty:'';?>}"/>
			</div>
			<input type="text" spellcheck="false"
				<?php $this->ng_attributes($option, 'change'); ?>
				ng-model="$parent.iframeScope.component.options[$parent.iframeScope.component.active.id]['model']['<?php echo esc_attr( $option ); ?>']<?php echo $deepProperty?$deepProperty:'';?>"
