            ...
            if ( !isBrake ) {
                ...
                if (undefined === searchId || item.id == searchId) {
                    ...
                    if(parseInt(item.id) < 100000) {
                        ...
                        if ( item.name != "oxy_header_left" && item.name != "oxy_header_right" && item.name != "oxy_header_center") {
                            var removeIcon = '<span class="ct-icon ct-delete-icon" title="Remove Component" ng-click="removeComponentWithUndo('+id+',\''+name+'\''+','+item.options['ct_parent']+')"></span>';
                        }
                        else {
                            var removeIcon = '';
                        }
                        ...
                        if ( item.id != 99999 && item.options.oxy_builtin != 'true' ) {
                            nodeOptions = '<div class="ct-node-options">'+
                                                '<span class="ct-icon ct-more-options-icon" ng-click="showCategorize=false"></span>'+
                                                removeIcon +
                                                    ...
                                                    '<div class="ct-more-options">'+
                                                        '<ul ng-show="showCategorize!=true">'+
                                                            ...
                                                            '<li ng-click="setEditableFriendlyName('+id+')">'+
                                                                'Rename'+
                                                            '</li>' + (angular.element('body').hasClass('ct_connection_active') && categoriesList ?
                                                            '<li ng-click="showCategorize=true">' +
                                                                'Categorize' +
                                                            '</li>': '') +
                                                        '</ul>'+ (categoriesList ? '<ul ng-show="showCategorize==true"><li ng-click="showCategorize=false">< Back</li>'+categoriesList+'</ul>': '')+
                                                    '</div>'+
                                                '</div>'+
                                            '</div>';
                        }
                        ...
                        if ( item.id != 99999 ) {
                            nodeDetails = '<span class="ct-node-details">'+
                                                '&lt;{{htmlTags["'+item.name+'"]}}&gt;'+
                                            '</span>';
                        }

                        DOMTree.HTML += 
                                '<div ng-attr-tree-actual-id="' + item.actualID + '" ng-attr-tree-id="' + item.id + '"' + ngClassNode + ' id="ct-dom-tree-node-' + item.id + '" class="ct-dom-tree-node ' + classes + '" ng-Style="parentScope.isActiveId('+id+')?{zIndex: 9999}:{}">' +
                                    ...
                                    '<div ng-attr-node-id="' + id + 
                                        ...
                                        '<div class="ct-dom-tree-node-header" '+
                                            'ng-class="{\'ct-handle\': editableFriendlyName!='+id+(angular.element('body').hasClass('ct_connection_active')?', \'categorized-component\': component.options[\''+item.id+'\'][\'ct_category\']':'')+'}">'+
