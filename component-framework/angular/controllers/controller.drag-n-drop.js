        ...
        $scope.iframeScope.$on('ct-dom-tree.drop', function (e, el, endParent, startParent) {
            ...
            if ( el.hasClass('ct-sortable-navigator-column') ) {
                ...
                if ( startParentId == endParentId ) {
                    $scope.iframeScope.rebuildDOM(startParentId);
                    $scope.iframeScope.rebuildDOM(endParentId);
                } 
                else {
                    $scope.iframeScope.rebuildDOM(startParentId);
                }
                ...
                //$scope.iframeScope.rebuildDOM(droppedItemId);
            }
            
            // if(angular.element(el).parent().attr('ng-attr-tree-id') !== endParentId) {
            //     if(endParentId > 0)
            //         $scope.iframeScope.updateDOMTreeNavigator(endParentId);
            //     else
