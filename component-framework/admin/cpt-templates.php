...
function ct_view_taxonomies_selector($field_name, $selected_items, $alloption = false) {
	?>
	<select name="<?php echo $field_name; ?>[]" id="<?php echo $field_name; ?>" multiple="multiple">
	...
	<script type="text/javascript">
		jQuery("#<?php echo $field_name; ?>").select2({
...
function ct_view_meta_box_callback( $post ) {
	...
	if ($screen->post_type != "ct_template") {
		...
		?>
			...
			<label style="margin-top: 10px;"><?php _e("Render Page Using Template", "component-theme");?><div class="oxy-tooltip"><div class="oxy-tooltip-text">Manage templates from the Oxygen » Templates screen.</div></div>
				...
				<select name="ct_other_template" id="ct_parent_template">
					<?php
						...
						<option <?php echo $contains_inner_content?'data-inner=true':'';?> value="0"  <?php echo empty($ct_other_template)?'selected':'';?> >Default (<?php echo sanitize_text_field( $generic_view->post_title ); ?>)</option>
						<?php
						}
						else {
							?>
						...
						foreach($templates as $template) {
							...
							?>
							<option <?php echo $contains_inner_content?'data-inner=true':'';?> value="<?php echo intval( $template->id ); ?>" <?php echo $is_selected_template?'selected':'';?>><?php echo sanitize_text_field( $template->post_title ); ?></option>
							<?php
			...
			<?php
				...
				<a <?php echo (!$show_edit_button && $option_is_selected)?'style="display: none" ':'';?>data-parent-template="<?php echo $ct_other_template?$ct_other_template:0;?>" href="<?php echo esc_url(ct_get_post_builder_link( $post->ID )); echo (($shortcodes && strpos($shortcodes, '[ct_inner_content') !== false) && intval($ct_other_template) !== -1)?'&ct_inner=true':''; ?>" id="ct-edit-template-builder" class="button button-primary">
					<?php printf( __("Edit with Oxygen", "component-theme") ); ?>
				</a>
	...
	// Button only for "Views"
		...
		$ct_template_type = get_post_meta( $post->ID, 'ct_template_type', true);
			...
			<div class='oxygen-edit-template-button-or-message'>
				<a data-parent-template="<?php echo $ct_parent_template?$ct_parent_template:0;?>" href="<?php echo esc_url(ct_get_post_builder_link( $post->ID )); echo ($shortcodes && strpos($shortcodes, '[ct_inner_content') !== false)?'&ct_inner=true':''; ?>" id="ct-edit-template-builder" class="button button-primary">
		...
		if($ct_template_type != 'reusable_part') {
			...
			<div class='oxygen-metabox-control-group'>
				<label style="margin-top: 10px;"><?php _e("Inherit design from other template", "component-theme");?>
					<br>
					<select name="ct_parent_template" id="ct_parent_template" data-parent-template="<?php echo (!$ct_parent_template || intval($ct_parent_template) === 0)?'0':$ct_parent_template;?>">
					...
					<div class="accordion-section-content ">
						<div class="inside">
							...
							<div class="oxygen-metabox-control-group">
								...
								<br>
								<input type="text" name="ct_template_post_of_parents" placeholder="<?php _e("Separate multiple page IDs with commas", "component-theme"); ?>" id="ct_template_post_of_parents" class='oxygen-vsb-metabox-input' value="<?php echo $template_post_of_parents;?>" />
							</div>
						</div>
					</div>
				</li>
...
// template type
	...
	// header/footer
	$template_header_footer_name = isset($_POST['ct_template_header_footer_name']) ? sanitize_text_field( $_POST['ct_template_header_footer_name'] ) : false;
